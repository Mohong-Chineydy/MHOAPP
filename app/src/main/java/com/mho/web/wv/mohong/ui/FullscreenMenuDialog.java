...
public class FullscreenMenuDialog extends Dialog {
    ...
    private View createContentView() {
        ...
        addButton(row1, "后退", v -> { if (listener != null) listener.onBack(); dismiss(); });
        addButton(row1, "前进", v -> { if (listener != null) listener.onForward(); dismiss(); });
        addButton(row1, "刷新", v -> { if (listener != null) listener.onRefresh(); dismiss(); });
        addButton(row1, "主页", v -> { if (listener != null) listener.onHome(); dismiss(); });
        layout.addView(row1);
        
        LinearLayout row2 = createButtonRow();
        addButton(row2, "分享", v -> { if (listener != null) listener.onShare(); dismiss(); });
        addButton(row2, "标签管理", v -> { if (listener != null) listener.onOpenTabManager(); dismiss(); });
        addButton(row2, "全屏", v -> { if (listener != null) listener.onFullscreenToggle(); dismiss(); });
        layout.addView(row2);
        
        LinearLayout row3 = createButtonRow();
        addButton(row3, "设置", v -> { if (listener != null) listener.onSettings(); dismiss(); });
        addButton(row3, "主题", v -> { if (listener != null) listener.onThemePicker(); dismiss(); });
        addButton(row3, "Cookie", v -> { if (listener != null) listener.onCookieManager(); dismiss(); });
        layout.addView(row3);
